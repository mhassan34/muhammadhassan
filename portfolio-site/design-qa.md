...
- State: home page, Splat selected, default pointer state
...
- Spacing and layout rhythm: header, three-column hero, numbered section label, left project rail, image panel, and transition into Experience align with the source at the same viewport. The archive was moved below Experience and expertise so it does not interrupt the recruiter flow.
...
- Copy and content: name, role, experience, technical focus, education, email, LinkedIn, Behance, and CV content are grounded in Muhammad Hassan's supplied CV and portfolio. The top project order is UMG, Splat, Multiplayer, with Splat selected by default.

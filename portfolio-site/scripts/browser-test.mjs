...
try {
  ...
  check("EOS dedicated server archive project", (await desktop.getByRole("heading", { name: "EOS Session & Dedicated Server Framework" }).count()) === 1);
  ...
  await desktop.getByRole("heading", { name: "EOS Session & Dedicated Server Framework" }).scrollIntoViewIfNeeded();
  await desktop.waitForTimeout(250);
  await desktop.screenshot({ path: "qa/implementation-final-project-row.png", fullPage: false });
